...
**GitHub**: https://github.com/ybotman/masterCalendarCollab

## Overview

**How It Works**:
- Git-based async messaging between agents
- JSON message files in individual inboxes
- Each agent has dedicated inbox + shared broadcast channel
- Background poller checks for new messages

**Available Agents**:
- sarah (Frontend - tangotiempo.com)
- ben (Backend - calendar-be Express)
- fulton (Backend - calendar-be-af Azure Functions)
- gotan (Backend - calendar-be-af Azure Functions)
- fred (DevOps/Infrastructure)
- donna (Documentation/PM)
- azule (Architecture/Design)
...
# Check your inbox (replace 'gotan' with your agent name)
ls -la inbox/gotan/

# Read a message
cat inbox/gotan/msg_20251021_150000_sarah_001.json

# Check broadcast
ls -la inbox/broadcast/
```
...
# Create message file
cat > inbox/sarah/msg_$(date +%Y%m%d_%H%M%S)_gotan_001.json <<'EOF'
{
  "from": "gotan",
  "to": "sarah",
  "subject": "MongoDB URI configuration fix",
  "message": {
    "summary": "Backend configured correctly. Frontend needs Vercel env vars.",
    "details": "PROD Azure Functions → TangoTiempoProd, TEST → TangoTiempo. Update NEXT_PUBLIC_AF_URL in Vercel."
  },
  "context": {
    "project": "TangoTiempo",
    "priority": "medium"
...
# Commit and push
git add inbox/sarah/
git commit -m "Message to Sarah: MongoDB URI config"
...
```json
{
  ...
  "context": {
    ...
    "relatedTickets": ["TIEMPO-322", "CALBEAF-54"]
  },
  "message": {
  ...
  "tags": ["mongodb", "deployment", "bug-fix"],
  "requestsResponse": true,
  "in_reply_to": "msg_20251019_140000_sarah_005.json"
}
```
...
| Agent | Role | Inbox Path |
|-------|------|------------|
| **sarah** | Frontend (Next.js) | `inbox/sarah/` |
| **ben** | Backend (Express) | `inbox/ben/` |
| **fulton** | Backend (Azure Functions) | `inbox/fulton/` |
| **gotan** | Backend (Azure Functions) | `inbox/gotan/` |
| **fred** | DevOps/Infrastructure | `inbox/fred/` |
| **donna** | Documentation/PM | `inbox/donna/` |
| **azule** | Architecture/Design | `inbox/azule/` |
| **broadcast** | All agents | `inbox/broadcast/` |
...
- When context from another agent needed
...
mv inbox/gotan/msg_*.json archive/$(date +%Y-%m-%d)/

git add inbox/ archive/
git commit -m "Archive: Processed messages from [agent-name]"
git push origin main
```
...
### Backend → Frontend (gotan → sarah)

**When**: Backend API changes, new endpoints, schema updates

```json
{
  "from": "gotan",
  "to": "sarah",
  "subject": "New Analytics Heatmap endpoint deployed",
  "message": {
    "summary": "GET /api/analytics/visitor-heatmap now available on PROD",
    "endpoint": "https://calendarbeaf-prod.azurewebsites.net/api/analytics/visitor-heatmap",
    "params": ["timeType (local|zulu)", "includeLogins (bool)", "includeVisitors (bool)"],
    "response": "7x24 heatmap matrix with traffic patterns",
    "swagger": "https://calendarbeaf-prod.azurewebsites.net/api/docs"
  },
  "context": {
    "relatedTickets": ["CALBEAF-54"]
  }
}
```

### Backend ↔ Backend (gotan → ben)

**When**: Migration coordination, schema questions, API parity

```json
{
  "from": "gotan",
  "to": "ben",
  "subject": "Venues API schema confirmation needed",
  "message": "Working on CALBEAF-42. Can you confirm Venues collection schema matches Express /api/venues response?",
  "context": {
    "relatedTickets": ["CALBEAF-42"]
  },
  "requestsResponse": true
}
```

### Any Agent → Architecture (gotan → azule)

**When**: Design decisions, pattern questions, architecture guidance
...
```json
{
  ...
  "message": "For User Logins import (CALBEAF-9), should we validate Firebase tokens for batch operations or use admin API?",
  "context": {
    "relatedTickets": ["CALBEAF-9"]
  },
  "priority": "high",
  "requestsResponse": true
}
...
**When**: Major updates, deployment announcements, breaking changes

```json
{
  "from": "gotan",
  "to": "broadcast",
  "subject": "PROD deployment: Analytics heatmap + MapCenter tracking",
  "message": {
    "summary": "New endpoints deployed to PROD",
    "endpoints": [
      "GET /api/analytics/visitor-heatmap",
      "POST /api/mapcenter/track"
    ],
...
1. **Check inbox at session start** - Always pull latest messages
2. **Use descriptive subjects** - Make scanning easy
3. **Include ticket references** - Link to JIRA/TIEMPO tickets
4. **Request responses when needed** - Set `requestsResponse: true`
5. **Archive processed messages** - Keep inbox clean
6. **Use broadcast sparingly** - Only for critical updates
7. **Be specific** - Include endpoints, schemas, examples
8. **Provide context** - Project, priority, related work

---

## Message-Aware Mode

Agents can run in **message-aware mode** where they:
- Poll inbox every 30 seconds
- Alert when new messages arrive
- Optionally pause for urgent messages
- Continue work for normal priority

**When to Enable**: Long-running tasks, async collaboration sessions

---

## Troubleshooting

**Problem**: Can't find agent-messages directory

**Solution**:
```bash
cd /Users/tobybalsley/Documents/AppDev/MasterCalendar
git clone https://github.com/ybotman/masterCalendarCollab.git agent-messages
```

**Problem**: Git conflicts when pushing messages
...
**Problem**: No response to message

**Solution**: Check if recipient agent is active, consider broadcast or contact BEAF directly
