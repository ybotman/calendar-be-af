...
| **`required: true` in swagger ≠ enforced + documented-intent-without-enforcement gaps (CALBEAF-177 Phase 1 audit + cross-handler audit 2026-05-11)** | Audit against TEST state 2026-05-07/11. Events_Create/Events_Update enforce: `appId`, `title`, `startDate`, `endDate`, `categoryFirstId` (CALBEAF-171 v1.34.0 via `validateCategoryFirstIdPresence`). **Silently optional gaps:** (a) **`venueID`** — missing → warn-logged at `Events.js:947`; event proceeds without `venueTimezone` (UC-0021 anchor); (b) **`ownerOrganizerID`** — missing → no 400; `authorOrganizerID` stays null (no immutable original-creator record; UC-0022 anchor; FULLY SILENT); (c) **`authorOrganizerID` UPDATE-path mutability (NEW cross-handler audit finding)** — Events.js:1100-1101 comment says *"immutable original creator ... should never change, even if ownerOrganizerID is reassigned later"* but Events_Update / Events_Patch handlers do NOT delete `authorOrganizerID` from update body; PUT body with `authorOrganizerID: <different-org-id>` would silently overwrite the immutable field. **EventsRA path IS protected** (`EventsRA.js:307 delete updateData.authorOrganizerID`); only public Events_Update path is the gap; (d) other niche-harvest/Porter-supplied fields pending caller survey (CALBEAF-177 Phase 2, Sprint 6). Cross-handler audit verified ENFORCED: TIEMPO-362 multi-day-vs-repeating on both Create + Update; `_id` immutable on Update (explicit delete); RA-path full delete-pattern for immutable fields; categoryFirstId Update-mode `must-not-clear` per CALBEAF-171; Organizers shortName uniqueness via validateOrFail; UserLogins required-field validation; Events_Spotlights `regionalOrganizerInfo.isApproved` post-CALBEAF-152-fix correct. | Spec assertions on Events POST/PUT: assume only the 5 fields above are enforced on Create; ALL fields except `categoryFirstId` are mutable on Update via public path. Test handler accepts missing required fields + accepts mutation of "immutable" `authorOrganizerID` via PUT body; observe silent downstream mangling. DO NOT assume "swagger says required → handler will reject" without checking handler source. Phase 2 tightening deferred to Sprint 6 per ticket defer-condition (3 anchor cases: UC-0021 venueID + UC-0022 ownerOrganizerID + NEW Events_Update authorOrganizerID gap). |
...
| v0.2.6 | 2026-05-11 | Fulton | §0.1 cross-handler audit fold (Quinn-task #20 lane-confirmed; 30-min grep pass per "deliver MORE" directive). Audit scope: `Venues.js / Organizers.js / UserLogins.js / EventsRA.js / Events_Spotlights.js / Events.js`. **NEW gap surfaced (3rd anchor for CALBEAF-177 Phase 2):** Events_Update path does NOT delete `authorOrganizerID` from update body; comment claims "should never change" but Update-path mutability is unenforced. EventsRA path IS protected (deletes immutable fields); only public Events_Update is the gap. Verified ENFORCED across all candidates: TIEMPO-362 multi-day-vs-repeating both paths / `_id` immutable on Update / RA delete-pattern complete / categoryFirstId update-mode-must-not-clear / Organizers shortName uniqueness / UserLogins required validation / Events_Spotlights post-CALBEAF-152 isApproved correct. Net audit findings: 3 documented-intent gaps (UC-0021 venueID + UC-0022 ownerOrganizerID + NEW authorOrganizerID Update-path); 7+ documented intents verified enforced. CALBEAF-177 Phase 2 scope expansion captured in §0.1 trap entry. Standby-gap codification per §18.4 (12th this Sprint 5 arc per running Quinn tally). |
